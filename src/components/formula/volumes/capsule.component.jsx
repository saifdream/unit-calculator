...
class Capsule extends Component {
    ...
    setResult() {
        if (this.state.radiusUnit === this.state.heightUnit) {
            this.setState({
                result: this.roundTo(4/3 * this.state.pi * Math.pow(this.state.radius, 3) + this.state.pi * Math.pow(this.state.radius, 2) * this.state.height),
                radiusUnitResult: 0,
                heightUnitResult: 0,
            });
        } else {
            let heightUnitToRadiusUnit = this.conversionSwitch(this.state.heightUnit + " to " + this.state.radiusUnit, this.state.height || 0);
            let radiusUnitToHeightUnit = this.conversionSwitch(this.state.radiusUnit + " to " + this.state.heightUnit, this.state.radius || 0);
            this.setState({
                result: 0,
                heightUnitToRadiusUnit: heightUnitToRadiusUnit,
                radiusUnitToHeightUnit: radiusUnitToHeightUnit,
                radiusUnitResult: 4/3 * this.state.pi * Math.pow(this.state.radius, 3) + this.state.pi * Math.pow(this.state.radius, 2) * parseFloat(heightUnitToRadiusUnit),
                heightUnitResult: 4/3 * this.state.pi * Math.pow(radiusUnitToHeightUnit, 3) + this.state.pi * Math.pow(parseFloat(radiusUnitToHeightUnit), 2) * this.state.height,
            });
        }
    }
    ...
    getResult() {
        if (this.state.radiusUnit === this.state.heightUnit) {
            return this.state.result ? <p style={{ fontSize: 20 }}>Volume = 4/3×π×{this.state.radius}³ + π×{this.state.radius}²×{this.state.height} = <strong>{this.state.result} {this.state.radiusUnit}³</strong></p> : '';
        } else {
            let radiusJsx = this.state.radiusUnitResult ? <p key="1" style={{ fontSize: 20 }}>Volume = 4/3×π×{this.state.radius}³ + π×{this.state.radius}²×{this.state.heightUnitToRadiusUnit} = <strong>{this.state.radiusUnitResult} {this.state.radiusUnit}³</strong></p> : '';
            let heightJsx = this.state.radiusUnitResult ? <p key="3" style={{ fontSize: 20 }}>Volume = 4/3×π×{this.state.radiusUnitToHeightUnit}³ + π×{this.state.radiusUnitToHeightUnit}²×{this.state.height} = <strong>{this.state.heightUnitResult} {this.state.heightUnit}³</strong></p> : '';
            ...
            <Form style={{ paddingTop: 30 }}>
                <Row>
                    <Col sm={12} style={{ textAlign: 'center', paddingBottom: 30 }}>
                        <Image src={require('../../../img/volumes/capsule.png')} rounded />
                    </Col>
                </Row>
                <Form.Group as={Row}>
