...
class Cone extends Component {
    ...
    setResult() {
        if (this.state.radiusUnit === this.state.heightUnit) {
            this.setState({
                result: this.roundTo(1 / 3 * this.state.pi * Math.pow(this.state.radius, 2) * this.state.height),
                radiusUnitResult: 0,
                heightUnitResult: 0,
            });
        } else {
            ...
            this.setState({
                ...
                radiusUnitResult: 1 / 3 * this.state.pi * Math.pow(this.state.radius, 2) * parseFloat(heightUnitToRadiusUnit),
                heightUnitResult: 1 / 3 * this.state.pi * Math.pow(parseFloat(radiusUnitToHeightUnit), 2) * this.state.height,
            });
        }
    }
    ...
    getResult() {
        if (this.state.radiusUnit === this.state.heightUnit) {
            return this.state.result ? <p style={{ fontSize: 20 }}>Volume = 1/3×π×{this.state.radius}²×{this.state.height} = <strong>{this.state.result} {this.state.radiusUnit}³</strong></p> : '';
        } else {
            let radiusJsx = this.state.radiusUnitResult ? <p key="1" style={{ fontSize: 20 }}>Volume = 1/3×π×{this.state.radius}²×{this.state.heightUnitToRadiusUnit} = <strong>{this.state.radiusUnitResult} {this.state.radiusUnit}³</strong></p> : '';
            let heightJsx = this.state.radiusUnitResult ? <p key="3" style={{ fontSize: 20 }}>Volume = 1/3×π×{this.state.radiusUnitToHeightUnit}²×{this.state.height} = <strong>{this.state.heightUnitResult} {this.state.heightUnit}³</strong></p> : '';
            ...
            <Form style={{ paddingTop: 30 }}>
                <Row>
                    <Col sm={12} style={{ textAlign: 'center', paddingBottom: 30 }}>
                        <Image src={require('../../../img/volumes/cone.png')} rounded />
                    </Col>
                </Row>
                <Form.Group as={Row}>
                    <Form.Label column sm="2">
                        <strong>Radius (<i>r</i>)</strong>
                    </Form.Label>
                    <Col sm="4">
